...
const AdminUserDetail = () => {
  ...
  return (
    <AdminLayout>
      <div className="mb-6">
        <Link to="/admin/users" className="text-slate-600 hover:text-indigo-600 font-medium text-sm flex items-center space-x-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-200 shadow-sm p-6 sm:p-8">
          <div className="flex items-center space-x-4 border-b border-slate-100 pb-6 mb-6">
            <div className="h-16 w-16 bg-indigo-100 text-indigo-700 flex items-center justify-center rounded-2xl font-bold text-2xl uppercase">
              {userDetail.name ? userDetail.name.substring(0, 2) : 'US'}
            </div>
            <div>
              <h3 className="text-xl font-bold text-slate-800">{userDetail.name}</h3>
              <p className="text-slate-500 text-sm">{userDetail.email}</p>
            </div>
          </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 flex flex-col justify-between">
          <div>
            <h3 className="text-md font-bold text-slate-800 border-b border-slate-100 pb-3 mb-4">Store Association</h3>
            {userDetail.role !== 'STORE_OWNER' ? (
              ...
              <div className="space-y-4">
                <div>
                  <h4 className="font-bold text-indigo-600 text-lg">{userDetail.store.name}</h4>
                  <p className="text-xs text-slate-500">{userDetail.store.email}</p>
                </div>
                ...
                <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                  <span className="text-sm font-semibold text-slate-500">Store Average Rating</span>
          ...
          {userDetail.role === 'STORE_OWNER' && userDetail.store && (
            <div className="pt-4 border-t border-slate-100 mt-6">
              <Link
                ...
                className="block text-center py-2 border border-slate-200 hover:bg-slate-50 text-indigo-600 font-semibold rounded-lg text-sm transition-colors"
              >
                View in Stores List
              </Link>
            </div>
